...
import pytz
from daily_verse_background_tasks import process_user
import asyncio
...
class verse_language_dropdown(discord.ui.Select):
    def __init__(self):
        options = [
...
class daily_verse_commmands(commands.Cog):
    def __init__(self, client):
        self.client = client

    @app_commands.command(name='setupdailyverse', description='Have the bot send you a random Quran verse in your DMs everyday')
    async def setupdailyverse(self, interaction: discord.Interaction):
        try:
            await interaction.response.defer()
            ...
            if await get_daily_data('DailyVerseData', user_id, 'VerseUserID') is not None:
                await interaction.followup.send("You already have the daily verse set up. If you would like to update any info use the `/updatedailyverse` command, or if you would like to set up the whole thing again use the `/disabledailyverse` command and *then* the `/setupdailyverse` commannd.")
                return
            
            instance = time_to_send_dropdown()
            view = dropdownView(instance)
            time_message = "Please select the time at which you want me to send the verse to you (Note that you might not get the verse at this *exact* time, just *around* this time):"
            ...
            if mode == 'Automatic':
                while True:
                    ...
                    if data['err'] == 'Error: Location not found':
                        ...
                        await interaction.followup.send('Automatic mode is not working right now. Please use the command again and use manual mode')
                        return
            ...
            instance = verse_language_dropdown()
            view = dropdownView(instance)
            language_message = "Please select the language you want to recieve the verse in:"
            ...
            await insert_daily_data('DailyVerseData', user_id, timezone, time_to_send, date, language)            
            await interaction.followup.send('All done :ballot_box_with_check:')
            asyncio.create_task(process_user(self.client, user_id))
        except Exception as e:
            print(f"\n\nERROR IN setupdailyverse COMMAND: {e}\n\n")    
        finally:
                users_list.remove(user_id)

    @app_commands.command(name='updatedailyverse', description='Update your daily verse info if you have it set up')
    @app_commands.choices(info_to_update = [
            discord.app_commands.Choice(name='Timezone (Automatic)', value='timezone_automatic'),
            discord.app_commands.Choice(name='Timezone (Manual)', value='timezone_manual'),
            discord.app_commands.Choice(name='Time at which you recieve the verse', value='time_to_send'),
            discord.app_commands.Choice(name='Verse Language', value='language')
        ])
    async def updatedailyverse(self, interaction: discord.Interaction, info_to_update: discord.app_commands.Choice[str]):
        try:
            await interaction.response.defer()
            ...
            if await get_daily_data('DailyVerseData', user_id, 'VerseUserID') is None:
                await interaction.followup.send("You do not have daily verse set up. To set up daily verse, use the `/setupdailyverse` command.")
                return
            ...
            if choice == 'timezone_automatic':
                while True:
                    ...
                    if choice == 'Yes':
                        ...
                        continue
                await update_daily_data('DailyVerseData', user_id, 'Timezone', timezone)
                ...
                if view is None:
                    ...
                    return
                time_to_send = view.children[0].time_to_send
                await update_daily_data('DailyVerseData', user_id, 'TimeToSend', time_to_send)
            else:
                instance = verse_language_dropdown()
                view = dropdownView(instance)
                language_message = "Please select the language you want to recieve the verse in:"
                ...
                await update_daily_data('DailyVerseData', user_id, 'Language', language)
            await interaction.followup.send('All done :ballot_box_with_check:')
        except Exception as e:
            print(f'\n\nERROR IN updatedailyverse COMMAND: {e}\n\n')
        finally:
            users_list.remove(user_id)

    @app_commands.command(name='disabledailyverse', description='Disable daily verse if you have it set up')
    async def disabledailyverse(self, interaction: discord.Interaction):
        try:
            await interaction.response.defer()
            user_id = interaction.user.id
            if await get_daily_data('DailyVerseData', user_id, 'VerseUserID') is None:
                await interaction.followup.send("You do not have daily verse set up. To set up daily verse, use the `/setupdailyverse` command.")
                return
            await delete_daily_data('DailyVerseData', user_id)
            await interaction.followup.send("Daily Verse has been disabled and your data has been deleted :wastebasket:")
        except Exception as e:
            print(f'\n\nERROR IN disabledailyverse COMMAND: {e}\n\n')

async def setup(client):
    await client.add_cog(daily_verse_commmands(client))